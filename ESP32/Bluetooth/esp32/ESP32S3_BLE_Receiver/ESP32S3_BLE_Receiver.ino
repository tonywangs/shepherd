...
static constexpr int kMotorForwardPin = 2;
static constexpr int kMotorReversePin = 3;

// Taptic pulse shape from the original Taptic.ino demo.
static constexpr uint32_t kCycleMicros = 3840;
static constexpr uint8_t kPulseCycles = 4;
...
static constexpr float kMotorTauSeconds = 0.7f;            // faster exponential decay time constant
...
void motorTask(void* parameter) {
  ...
  while (true) {
    ...
    const int pwm = static_cast<int>(roundf(fabsf(outputNorm) * 255.0f));
